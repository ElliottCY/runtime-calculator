...
def runtime_calculator(start, end, hourly_solar, bifacial_gain, load, battery_capacity, battery_rate, inverter_rate, generator_output):
    
    runtime = 0
    if(start == end):
        hours = 8760
    else:
        hours = (8760 + end - start)%8760
    ...
    for j in range(hours):
        ...
        if(hourly_load - solar <= 0 and (not generator_on)): #solar exceeds load, battery gets charged
            battery_state += battery_efficiency*min(charge_rate, (solar - hourly_load))
            battery_state = min(battery_state, battery_capacity)
            generator_on=False
            
            
        elif((not generator_on) and (battery_state - (hourly_load - solar)) > low*battery_capacity and rate >= (hourly_load - solar)/inverter_efficiency): #battery can cover the difference, generator not needed
