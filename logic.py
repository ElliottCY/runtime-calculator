def runtime_calculator(hourly_solar, bifacial_gain, load, battery_capacity, battery_rate, inverter_rate, generator_output):
    runtime = 0
    hours = 8760
    ...
    low = 0.2
    high = 0.8
    rate = min(battery_rate, inverter_rate)
    ...
    for i in range(hours):
        ...
        if(hourly_load - solar <= 0): #solar exceeds load, battery gets charged
            battery_state += battery_efficiency*min(rate, (solar - hourly_load))
            ...
            while(temp < 60): #cycles through every minute of an hour that uses the generator
                ...
                if(generator_on):
                    battery_state += battery_efficiency*min(rate, inverter_efficiency * (generator_output - hourly_load + solar))/60
                    battery_state = min(battery_state, battery_capacity)
                    runtime += 1/60
                else:
